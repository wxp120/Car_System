...
void Environment::startMonitoring()
{
    ...
    if (lightTimer) lightTimer->start(1000);
...
bool Environment::readMQ135RawValue(int &rawValue)
{
    ...
    bool ok;
    rawValue = data.trimmed().toInt(&ok);
    if (!ok) {
        qDebug() << "Failed to parse raw value:" << data;
        rawValue = 0;
        return false;
    }
...
// 全局文件句柄，只打开一次！和你C程序一样！
static QFile g_dht11File("/dev/dht11");

void Environment::readDHT11Data(float &temp, float &humidity)
{
    // 第一次打开，只打开一次！
    if (!g_dht11File.isOpen())
    {
        if (!g_dht11File.open(QIODevice::ReadOnly))
        {
            qDebug() << "打开 /dev/dht11 失败";
            temp = 0.0;
            humidity = 0.0;
            return;
        }
    ...
    // 直接读，不关闭！和你C程序一模一样！
    char buf[16] = {0};
    qint64 len = g_dht11File.read(buf, 5);
...
void Environment::setupLightChart()
{
    ...
    lightChartView->setRenderHint(QPainter::Antialiasing);
}
